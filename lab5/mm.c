...
void* mm_malloc(size_t size) {
  ...
  //if there is not enough space in the heap, request at least req_size more free space
  ...
  //get the preceding block_use_tag from the free block
  preceding_block_use_tag = (ptr_free_block -> size_and_tags) & TAG_PRECEDING_USED;
  //remove the block from the free list
  remove_free_block(ptr_free_block);
  // if the left over of the free list is greater than the minimum block size
  // split the free block and store the left-over block to the free list
  if(block_size >= req_size + MIN_BLOCK_SIZE){
    //get the size of the remaining part after allocating the requested size
    size_t remaining_size = block_size - req_size;
    //get the address of the new free block
    block_info *remaining_block = (block_info *)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    //get the header of the new free list
    remaining_block->size_and_tags = remaining_size | TAG_PRECEDING_USED;
    //set the footer of the new free list
     *((size_t *)UNSCALED_POINTER_ADD(remaining_block, remaining_size - WORD_SIZE)) = remaining_size | TAG_PRECEDING_USED;
    //insert the new free list to the free list
    insert_free_block(remaining_block);
  } else {
    //if the residue of the block is smaller than the minimum block size
    //add it as the padding of the allocated block
    req_size = block_size;
    //update the following block's header to 1 indicating is now used
    block_info* following_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    following_block->size_and_tags = following_block->size_and_tags | TAG_PRECEDING_USED;
  }
  //set the new header of the block
...
void mm_free(void* ptr) {
  ...
  //get pointer to the header of the block
  block_to_free = (block_info*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  //get the size of the payload
  payload_size = SIZE(block_to_free->size_and_tags) - WORD_SIZE;

  //set the used tag of the header of the block_to_free to 0, means not used
  block_to_free->size_and_tags = block_to_free->size_and_tags & ~TAG_USED;

  //make the footer of the block_to_free to be the same as the header
  *(size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size) = block_to_free -> size_and_tags;

  //clear the TAG_PRECEDING_USED in the header of the following block, make it 0, means not used
  following_block = (block_info*)UNSCALED_POINTER_ADD(block_to_free,SIZE(block_to_free->size_and_tags));
  following_block->size_and_tags = following_block -> size_and_tags & ~TAG_PRECEDING_USED;
  //update the footer of the following block if it's free
  if ((following_block->size_and_tags & TAG_USED) == 0) {
      *(size_t*) UNSCALED_POINTER_ADD(following_block, SIZE(following_block->size_and_tags) -
      WORD_SIZE) = following_block->size_and_tags;
  }
  //instert the block into the free list and coalesce the block to free
