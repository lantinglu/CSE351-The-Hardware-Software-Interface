...
unsigned short get_section(unsigned long* aisle, int index) {
  ...
  unsigned short sec = (*aisle >> (SECTION_SIZE * index))& SECTION_MASK;
  return sec;
}
...
unsigned short get_spaces(unsigned long* aisle, int index) {
  // TODO: implement this method
  unsigned short sec = get_section(aisle, index);
  unsigned spa = sec & SPACES_MASK;
  return spa;
}
...
unsigned short get_id(unsigned long* aisle, int index) {
  // TODO: implement this method
  unsigned short sec = get_section(aisle, index);
  unsigned short id = (sec & ID_MASK) >> NUM_SPACES;
  return id;
}

/* Given a pointer to an aisle, a section index, and a short representing a new
 * bit patt
 * ern to be used for section, set the section at the given index of
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
  // TODO: implement this method
  unsigned long sec_clear = ~((unsigned long)SECTION_MASK << (SECTION_SIZE * index));
  *aisle &= sec_clear;
  unsigned long new_sec= (unsigned long)new_section << (SECTION_SIZE * index);
  *aisle |= new_sec;
}
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  ...
  unsigned long space_clear = ~((unsigned long)SPACES_MASK << (index * SECTION_SIZE));
  *aisle &= space_clear;
  unsigned long new_spa = (unsigned long)new_spaces << (index * SECTION_SIZE);
  *aisle |= new_spa;
}
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  ...
  unsigned long sec_clear = ~((unsigned long)ID_MASK << (index * SECTION_SIZE));
  *aisle &= sec_clear;
  unsigned long id_new = (unsigned long)new_id << (index * SECTION_SIZE + NUM_SPACES);
  *aisle |= id_new;
}
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  // TODO: implement this method
  unsigned short sec = get_section(aisle, index);
  unsigned short newSec = sec ^ 1 << space_index;
  set_section(aisle, index, newSec);
}
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  unsigned short left_spaces = ((spaces << n) | (spaces >> (NUM_SPACES - n))) & SPACES_MASK;
  set_spaces(aisle,index,left_spaces);
}
...
void rotate_items_right(unsigned long* aisle, int index, int n) {
  ...
  unsigned short spaces = get_spaces(aisle, index);
  unsigned short right_spaces = ((spaces >> n) | (spaces << (NUM_SPACES - n))) & SPACES_MASK; 
  set_spaces(aisle,index,right_spaces);
}
