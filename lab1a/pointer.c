...
int within_same_block(int* ptr1, int* ptr2) {
  // Your code here
  long int add1 = (long int)ptr1;
  long int add2 = (long int)ptr2;
  return (add1 >> 6) == (add2 >> 6);
}
...
int within_array(int* int_array, int size, int* ptr) {
  // Your code here
  int diff = ptr - int_array;
  int begin = diff >>31;
  int end = (size - 1 - diff)>>31;
  return !(begin^end); 
}
 ...
 *   Binary integer operators: -, +, *, ==, !=, <, >
 *   Unary integer operators: !
 *   Shorthanid operators based on the above: +=, *=, ++, --, etc.
